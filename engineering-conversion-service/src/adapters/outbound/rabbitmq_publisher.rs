...
impl RabbitMqPublisher {
    ...
    async fn connect(config: &AppConfig) -> Result<PublisherState, DomainError> {
        ...
        let connection = Connection::connect(&config.amqp_url, ConnectionProperties::default())
            .await
            .map_err(|e| DomainError::ConfigReadError("AMQP_URL".to_string(), format!("Publisher connection failed: {e}")))?;

        let channel = connection.create_channel().await.map_err(|e| {
            DomainError::ConfigReadError("AMQP_URL".to_string(), format!("Publisher channel creation failed: {e}"))
        })?;
        ...
        channel
            ...
            .await
            .map_err(|e| {
                DomainError::ConfigReadError("AMQP_URL".to_string(), format!("Publisher confirm_select failed: {e}"))
            })?;
        ...
        channel
            ...
            .await
            .map_err(|e| {
                DomainError::ConfigReadError("DESTINATION_EXCHANGE".to_string(), format!("Publisher exchange_declare failed: {e}"))
            })?;
...
impl EngineeringPublisher for RabbitMqPublisher {
    async fn publish(&self, envelope: &TelemetryEnvelope, routing_key: &str) -> Result<(), DomainError> {
        ...
        envelope
            .encode(&mut bytes)
            .map_err(|e| DomainError::EvaluationError(envelope.envelope_id.clone(), format!("Failed to encode envelope: {e}")))?;
        ...
        loop {
            ...
            if attempts >= max_attempts {
                return Err(DomainError::EvaluationError(
                    ...
                    format!("Failed to publish and confirm envelope after {} attempts", max_attempts),
                ));
            }
