...
//
// Responsibility: Connect to RabbitMQ, declare queue/exchange/routing key,
// and drive the message consume loop.
//
// This is the implementation of the driving port `EnvelopeConsumer`.
// It maps lapin/AMQP errors to `DecoderError::AmqpError`.
...
use crate::ports::inbound::{DeliveryAcker, EnvelopeConsumer, HandlerFn};

/// Internal type representing an active AMQP delivery acker.
struct LapinAcker {
    channel: Channel,
    delivery_tag: u64,
}
...
#[async_trait]
impl crate::ports::inbound::AckerInner for LapinAcker {
    ...
    async fn nack(&mut self) {
        // requeue = false -> drop or route to DLX (dead letter exchange) if configured
...
impl EnvelopeConsumer for RabbitMqConsumer {
    async fn start(&self, handler: HandlerFn) -> Result<(), DecoderError> {
        ...
        let conn = Connection::connect(&self.config.amqp_url, ConnectionProperties::default())
            .await
            .map_err(|e| DecoderError::AmqpError(format!("Failed to connect: {e}")))?;

        tracing::info!("RabbitMQ connection established. Creating channel...");
        let channel = conn
            .create_channel()
            .await
            .map_err(|e| DecoderError::AmqpError(format!("Failed to create channel: {e}")))?;
        ...
        channel
            ...
            .await
            .map_err(|e| DecoderError::AmqpError(format!("Failed to set basic QOS: {e}")))?;
        ...
        let mut consumer = channel
            ...
            .await
            .map_err(|e| DecoderError::AmqpError(format!("Failed to start consumer: {e}")))?;
        ...
        while let Some(delivery_result) = consumer.next().await {
            match delivery_result {
                ...
                Err(e) => {
                    ...
                    // Let's return error to trigger reconnection (or fail-fast)
